...
#define WIFI_LED_PIN 8  // 新增WiFi狀態指示燈
...
// WiFi设置
const char* ssid = "COFFEE.HPA.FCU";  // 預設AP名稱
const char* password = "12345678";    // 預設AP密碼
const char* knownSSIDs[] = {"COFFEE.HPA.FCU", "A", "B"};  // 已知WiFi列表
const char* knownPasswords[] = {"12345678", "00001111", "00000000"};  // 對應密碼
WebServer server(80);
...
void doubleAction();
void customBurst();
void connectToWiFi();  // 新增WiFi連接函數
void startAPMode();    // 新增AP模式啟動函數
void blinkWiFiLED(int times, int delayTime);  // 新增LED指示函數
...
void connectToWiFi() {
  ...
  for (int i = 0; i < sizeof(knownSSIDs)/sizeof(knownSSIDs[0]); i++) {
    for (int j = 0; j < n; j++) {
      if (strcmp(knownSSIDs[i], WiFi.SSID(j).c_str()) == 0) {
        ...
        int attempts = 0;
        while (WiFi.status() != WL_CONNECTED && attempts < 20) {  // 嘗試20次
          delay(500);
          Serial.print(".");
          digitalWrite(WIFI_LED_PIN, !digitalRead(WIFI_LED_PIN));  // 閃爍表示正在連接
          attempts++;
        }
        ...
        if (WiFi.status() == WL_CONNECTED) {
          ...
          digitalWrite(WIFI_LED_PIN, HIGH);  // 常亮表示已連接
          WiFi.scanDelete();  // 釋放掃描結果記憶體
          return;
        } else {
          Serial.println("\nFailed to connect");
          WiFi.disconnect();  // 斷開連接
          blinkWiFiLED(5, 200);  // 快速閃爍5次表示連接失敗
        }
      }
    }
  }
...
void startAPMode() {
  ...
  while (true) {
    ...
    if (millis() - lastTryTime > 30000) {  // 每30秒嘗試一次
      lastTryTime = millis();
...
// 新增LED指示函數
void blinkWiFiLED(int times, int delayTime) {
